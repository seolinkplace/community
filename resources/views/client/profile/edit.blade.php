...
@section('content')
<h1 class="text-2xl font-semibold text-gray-900 mb-6">{{ __('client.profile_title') }}</h1>

<div class="max-w-2xl space-y-6">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h2 class="font-medium text-gray-900 mb-4">Основні дані</h2>
        <form method="POST" action="{{ route('client.profile.update') }}">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Тариф</label>
                    <span class="inline-block px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-sm capitalize">{{ $client->plan }}</span>
                </div>
                @if($client instanceof \App\Models\UnifiedUser)
                <div>
                    ...
                    <select name="locale" class="w-full border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                        <option value="uk" {{ old('locale', $client->locale) === 'uk' ? 'selected' : '' }}>{{ __('client.lang_uk') }}</option>
                        <option value="en" {{ old('locale', $client->locale) === 'en' ? 'selected' : '' }}>{{ __('client.lang_en') }}</option>
                    </select>
                </div>
                @endif
            </div>
            <button type="submit" class="mt-4 bg-gray-900 text-white px-6 py-2 rounded-lg text-sm hover:bg-gray-700">{{ __('client.save') }}</button>
        </form>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h2 class="font-medium text-gray-900 mb-4">Змінити пароль</h2>
        <form method="POST" action="{{ route('client.profile.password') }}">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Підтвердити</label>
                    <input type="password" name="password_confirmation"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                </div>
            </div>
            <button type="submit" class="mt-4 bg-gray-900 text-white px-6 py-2 rounded-lg text-sm hover:bg-gray-700">Змінити пароль</button>
        </form>
    </div>
</div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <form method="POST" action="{{ route('unified.add.role') }}">
                ...
                <button type="submit"
                    class="flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-red-200 dark:border-red-900 p-6">
        ...
        <form method="POST" action="{{ route('unified.account.delete') }}"
              onsubmit="return confirm('{{ __('auth.delete_account_title') }}?')">
            @csrf @method('DELETE')
