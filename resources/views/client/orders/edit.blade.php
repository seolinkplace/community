...
@section('title', 'Редагувати розміщення')
@section('content')
<div class="max-w-2xl mx-auto py-8 px-4">

    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('client.orders.index') }}" class="text-gray-400 hover:text-gray-600 text-sm">← Замовлення</a>
        <span class="text-gray-300">/</span>
        <h1 class="text-xl font-bold text-gray-900">Редагувати розміщення</h1>
    </div>
    ...
    <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 mb-5 text-sm text-gray-600">
        <div>Кампанія: <strong>{{ $link->campaign?->name ?? '—' }}</strong></div>
        <div>Сайт: <strong>{{ $link->site?->domain ?? '—' }}</strong></div>
        <div>Ціна: <strong>${{ number_format($link->price_per_day, 2) }}/день</strong> <span class="text-gray-400 text-xs">(виставляє вебмастер)</span></div>
    </div>

    <form method="POST" action="{{ route('client.orders.update', $link) }}"
    ...
    {{-- Status history --}}
    <div class="mt-6 bg-white rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
