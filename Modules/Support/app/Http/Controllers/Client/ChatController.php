...
class ChatController extends Controller
{
    public function index()
    {
        ...
        $links = CampaignLink::whereHas("campaign", fn($q) => $q->where("client_id", $client->id))
            ->whereHas("messages")
            ->with(["site", "campaign", "messages" => fn($q) => $q->latest()->limit(1)])
            ->get()
            ->map(function($link) {
                $unread = ChatMessage::where("campaign_link_id", $link->id)
                    ->where("sender_type", "webmaster")
                    ->whereNull("read_at")
                    ->count();
        ...
        return view("client.chat.index", compact("links"));
    }
    ...
    public function show(CampaignLink $campaignLink)
    {
        ...
        // Позначаємо повідомлення вебмастера як прочитані
        ChatMessage::where('campaign_link_id', $campaignLink->id)
            ->where('sender_type', 'webmaster')
            ->whereNull('read_at')
            ->update(['read_at' => now()]);

        return view('client.chat.show', compact('campaignLink', 'messages'));
    }
    ...
    {
        $clientId = $link->campaign?->client_id;
        if ($clientId !== \App\Helpers\AuthHelper::clientId()) {
            abort(403);
        }
    }
